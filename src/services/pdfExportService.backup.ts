...
//
// PDF_USABLE_HEIGHT_PX is intentionally set below the visible A4 content zone (930px)
// to account for the 1.60–1.67× font-scale inflation that html2canvas applies when
// capturing the clone (PDF_FONT_SCALE=1.45 + line-height reflow). Debug logging
// confirmed that measured 930px blocks render at ~1468px, clipping 345px of content.
// Formula: 930 / 1.67 ≈ 557px → 570px gives safety margin while keeping terms (561px) on 1 page.
const PDF_USABLE_HEIGHT_PX = 900;
...
interface MeasuredBlock {
  ...
  /** True when the block contains at least one <img> element.
   *  Image blocks inflate ~1.12x under html2canvas; text blocks inflate ~1.67x.
   *  Used by effectiveBlockHeight() to pick the right inflation factor. */
  hasImages: boolean;
...
// ---------------------------------------------------------------------------
// Per-block inflation: estimated rendered height for page-fit decisions
// ---------------------------------------------------------------------------
// Text blocks inflate ~1.67x under html2canvas (PDF_FONT_SCALE=1.45 + line-height reflow).
// Image blocks inflate ~1.12x (fixed-pixel height, minimal text reflow).
const TEXT_INFLATION = 1.67;
const IMAGE_INFLATION = 1.12;

/** Returns the estimated rendered height of a block after html2canvas font scaling. */
const effectiveBlockHeight = (b: MeasuredBlock): number => {
  const inflation = b.hasImages ? IMAGE_INFLATION : TEXT_INFLATION;
  return Math.ceil((b.height + b.marginBottom) * inflation);
};
...
const computeVirtualPages = (
  ...
  for (let i = 0; i < blocks.length; i++) {
    ...
    const strictFit = current.totalContentHeight + blockHeight <= usableHeight;
    const softFit = !strictFit
      && current.blockIndices.length > 0
      && current.totalContentHeight + blockHeight <= usableHeight * 1.10;
    const wouldFit = strictFit || softFit;
...
const computeVirtualPagesWithTableRows = (
  ...
  for (let i = 0; i < blocks.length; i++) {
    ...
    if (block.type !== 'table' || !block.tableData) {
      ...
      // Keep-with-next: if this atomic fits but the immediately following block is a table,
      // check whether (this atomic + table minimum unit: thead + first row) fits together.
      // If not, flush the current page BEFORE placing this atomic so a service heading
      // and its pricing table always start on the same page — prevents orphaned headings.
      // Only applies when the current page already has content (never flush a fresh empty page).
      if (current.blockIndices.length > 0) {
        const nextBlock = blocks[i + 1];
        if (nextBlock && nextBlock.type === 'table' && nextBlock.tableData) {
          const minTableUnit = nextBlock.tableData.theadHeight + (nextBlock.tableData.rows[0]?.height ?? 0);
          if (current.totalContentHeight + blockHeight + minTableUnit > usableHeight) {
            // Heading + table minimum unit won't fit together on the current page — flush first
            pages.push(current);
            current = { blockIndices: [i], totalContentHeight: blockHeight };
            continue;
          }
    ...
    // is a missing TOTAL.
    const TABLE_FAST_PATH_SAFETY_PX = tfootHeight + 20;
    if (
...
const captureVirtualPage = async (
  ...
  try {
    // Hide ALL footers from inline flow — merged pdf-page-services contains one footer per service.
    // querySelector would only hide the first; querySelectorAll hides every instance so none
    // render as visible content blocks inside a virtual page.
    clone.querySelectorAll<HTMLElement>('.company-contact-footer').forEach(el => {
    ...
    if (isMultiService) {
      ...
      const captureSmartSection = async (
        ...
        if (blocks.length > 0) {
          ...
          // ── DEBUG CONSOLE ── shows page assignments, block indices, heights, text snippets.
          // Remove this block before going to production.
          {
            const allBlockEls = Array.from(document.querySelectorAll(`#${sectionId} [data-pdf-block]`));
            console.group(`📐 PDF BLOCK LAYOUT — ${sectionId} (${blocks.length} blocks → ${vPages.length} pages) | usable: ${PDF_USABLE_HEIGHT_PX}px`);
            vPages.forEach((vp, pi) => {
              const fill = Math.round((vp.totalContentHeight / PDF_USABLE_HEIGHT_PX) * 100);
              const segInfo = vp.tableSegment ? ` [table rows ${vp.tableSegment.startRow}–${vp.tableSegment.endRow}]` : '';
              console.group(`▶ PAGE ${pi + 1} — ${Math.round(vp.totalContentHeight)}px / ${PDF_USABLE_HEIGHT_PX}px (${fill}% full)${segInfo}`);
              vp.blockIndices.forEach(bi => {
                const b = blocks[bi];
                const el = allBlockEls[bi] as HTMLElement | undefined;
                const rawText = el?.textContent?.replace(/\s+/g, ' ').trim().slice(0, 60) || '(no text)';
                console.log(`B${bi} [${b.type}] h=${b.height}px margin=${Math.round(b.marginBottom)}px | ${rawText}`);
              });
              console.groupEnd();
            });
            const assignedIndices = new Set(vPages.flatMap(vp => vp.blockIndices));
            const dropped = blocks.filter((_, bi) => !assignedIndices.has(bi));
            if (dropped.length > 0) {
              console.warn(`⚠ ${dropped.length} DROPPED / UNASSIGNED BLOCK(S):`);
              dropped.forEach(b => {
                const el = allBlockEls[b.index] as HTMLElement | undefined;
                const rawText = el?.textContent?.replace(/\s+/g, ' ').trim().slice(0, 60) || '(no text)';
                console.warn(`  B${b.index} [${b.type}] ${b.height}px | ${rawText}`);
              });
            }
            console.groupEnd();
          }
          // ── DEBUG: measured vs rendered comparison per virtual page ──────────────────
          // Real html2canvas height vs measured DOM height — shows exact footer overflow.
          // Remove before production.
          const _debugRenderLog2: Array<{
            vpIndex: number; blockIds: string; measuredPx: number;
            canvasCssPx: number; overflowBy: number; blockSummary: string[];
          }> = [];
          const _allBlockEls2 = Array.from(document.querySelectorAll(`#${sectionId} [data-pdf-block]`));

          for (let vp = 0; vp < vPages.length; vp++) {
            console.log(`Rendering virtual page ${vp + 1}/${vPages.length}`, { sectionId, currentPdfPage: pdf.getCurrentPageInfo().pageNumber });
            const vpResult = await captureVirtualPage(sectionId, vPages[vp], blocks);
            if (vpResult) {
              // canvas rendered at scale=2, divide by 2 for CSS px equivalent
              const canvasCssPx2 = Math.round(vpResult.canvas.height / 2);
              const overflowBy2 = canvasCssPx2 - PDF_USABLE_HEIGHT_PX;
              const blockSummary2 = vPages[vp].blockIndices.map(bi => {
                const b = blocks[bi];
                const el = _allBlockEls2[bi] as HTMLElement | undefined;
                const name = el?.textContent?.replace(/\s+/g, ' ').trim().slice(0, 50) || '(no text)';
                return `    B${bi} [${b.type}] measured=${b.height}px margin=${Math.round(b.marginBottom)}px | name="${name}"`;
              });
              _debugRenderLog2.push({
                vpIndex: vp + 1,
                blockIds: vPages[vp].blockIndices.join(','),
                measuredPx: Math.round(vPages[vp].totalContentHeight),
                canvasCssPx: canvasCssPx2,
                overflowBy: overflowBy2,
                blockSummary: blockSummary2,
              });
              addCanvasToPDF(vpResult.canvas, vpResult.links, `${label}-vp${vp}`, isFirstOfDoc && vp === 0);
            } else {
              console.warn(`Virtual page ${vp + 1} returned null — skipped`, { sectionId });
            }
          }

          // Print measured vs rendered table
          console.group(`📊 MEASURED vs RENDERED — ${sectionId} (${_debugRenderLog2.length} pages)`);
          _debugRenderLog2.forEach(r => {
            const diff = r.canvasCssPx - r.measuredPx;
            const status = r.overflowBy > 0
              ? `⚠️  OVERFLOW +${r.overflowBy}px BLEEDS INTO FOOTER`
              : r.overflowBy < -150 ? `🟡 ${Math.abs(r.overflowBy)}px empty space` : `✅ ok`;
            console.group(`PAGE ${r.vpIndex} | blocks[${r.blockIds}] | measured=${r.measuredPx}px → rendered=${r.canvasCssPx}px (+${diff}px font-scale inflation) | ${status}`);
            r.blockSummary.forEach(line => console.log(line));
            console.groupEnd();
          });
          console.groupEnd();
          // ── END DEBUG ──
      ...
      // Capture all service pages — prefer the merged wrapper (pdf-page-services) so the
      // smart block packer can flow services continuously without forced page breaks.
      // Falls back to the old per-service loop when the wrapper is not found (e.g. other templates).
      const mergedServicesEl = document.getElementById('pdf-page-services');
      if (mergedServicesEl) {
        // Wait for ALL ReferenceImages inside the merged section to be ready before capturing.
        // The old waitForPdfReady only waited for the first [data-pdf-ready]; here we wait for all.
        const allReadyEls = Array.from(mergedServicesEl.querySelectorAll('[data-pdf-ready]'));
        await Promise.all(allReadyEls.map(el =>
          new Promise<void>(resolve => {
            const poll = setInterval(() => {
              if (el.getAttribute('data-pdf-ready') === 'true') { clearInterval(poll); resolve(); }
            }, 150);
            setTimeout(() => { clearInterval(poll); resolve(); }, 15000);
          })
        ));
        await captureSmartSection('pdf-page-services', 'services');
      } else {
        // Old per-service loop — kept as fallback for other templates
        let serviceIndex = 0;
        while (true) {
