...
export async function parseChatIntentWithAi(
  ...
  try {
    const prompt = [
      ...
      '- kind=services_browse when the user asks to list or show available services.',
      '- Treat "list all", "show all", "all services", "list all services", and "what services are available?" as services_browse with media=[].',
      '- For scoped lists like "list auto services" or "show bus services", use services_browse with media set to that family token (e.g. media=["auto"]).',
      '- For unscoped services_browse, return media=[], city=null, areaHint=null, directionHint=null, ambiguous=false.',
      ...
      '  "list auto services" → kind=services_browse, media=["auto"], city=null, ambiguous=false',
