...
const PDF_FONT_LOAD_SPECS = ['400 14px Calibri', '700 14px Calibri'];
 ...
 * Many templates declare body text at 911 px  6.758.25 pt (unreadable).
 *
 ...
 *   body floor  10.5 pt    body  12 pt    titles  2230 pt
 *
...
const ensurePdfFontLoaded = async (): Promise<void> => {
  ...
  try {
    ...
    console.warn(' PDF font preload failed, continuing with fallback fonts:', error);
  }
};
...
// ---------------------------------------------------------------------------
// Smart Block-Based Pagination  Stage 1: Measurement Engine
// ---------------------------------------------------------------------------
...
interface MeasuredBlock {
  ...
  height: number; // px  content height from getBoundingClientRect
  marginBottom: number; // px  bottom spacing between this block and the next
...
interface TableSegment {
  ...
  isFirstSegment: boolean; // false  context blocks before table removed in clone
}

/** A group of block indices that fit together on one physical PDF page */
interface VirtualPage {
  blockIndices: number[];
  totalContentHeight: number; // px  sum of (height + marginBottom) for all blocks on this page
...
const captureSectionAtA4 = async (
  ...
  if (!source) {
    console.warn(` Section ${containerId} not found`);
    return null;
  }

  logger.debug(` Capturing section: ${containerId}`);
  ...
  try {
    ...
    logger.debug(` ${images.length} images loaded`);
    ...
    const canvas = await html2canvas(clone, {
      ...
      onclone: (doc, clonedEl) => {
        ...
        logger.debug(
          '[PDF] onclone (captureSectionAtA4): ' + links.length + ' links from ' + containerId,
        );
      },
    });

    logger.debug(` Section captured: ${canvas.width}x${canvas.height}px`);
...
const compositeWithPageFooter = async (
  ...
  try {
    ...
    logger.debug('[PDF] compositeWithPageFooter: ' + footerLinks.length + ' footer links');
...
const measureSectionBlocks = async (containerId: string): Promise<MeasuredBlock[]> => {
  const source = document.getElementById(containerId);
  if (!source) {
    console.warn(` measureSectionBlocks: section "${containerId}" not found`);
    return [];
  }

  if (source.querySelectorAll('[data-pdf-block]').length === 0) {
    logger.debug(
      ` measureSectionBlocks: no [data-pdf-block] elements in "${containerId}", skipping`,
    );
    return [];
  }
  ...
  try {
    await document.fonts.ready;
    // Two rAF cycles guarantee layout is fully resolved  more reliable than a fixed timeout
    ...
    logger.debug(
      ` measureSectionBlocks("${containerId}"): ${measured.length} blocks measured`,
      measured,
    );
    return measured;
  } finally {
 ...
 *
 * Rules:
 *   - If a block fits in remaining space   place on current page
 *   - If a block does not fit             flush current page, start new page
 *   - header-group orphan prevention      never leave a heading alone at the
 *     bottom of a page without its following sibling
 */
const computeVirtualPages = (blocks: MeasuredBlock[], usableHeight: number): VirtualPage[] => {
  ...
  for (let i = 0; i < blocks.length; i++) {
    ...
    if (!wouldFit) {
      // Block does not fit  flush current page and start a new one
    ...
    // header-group orphan rule: heading fits but its next sibling would not
    ...
    if (block.type === 'header-group' && i + 1 < blocks.length) {
      ...
      if (
        !nextFitsAfterHeader &&
        current.blockIndices.length > 0 &&
        current.totalContentHeight > 0
      ) {
        pages.push(current);
        current = { blockIndices: [i], totalContentHeight: blockHeight };
        continue;
...
const computeVirtualPagesWithTableRows = (
  ...
  for (let i = 0; i < blocks.length; i++) {
    ...
    //  Non-table blocks: same rules as computeVirtualPages
    ...
    //  Table block: row-aware pagination
    ...
    while (rowCursor < rows.length) {
      ...
      for (let r = rowCursor; r < rows.length; r++) {
        ...
        if (tally + cost <= rowBudget) {
          ...
          // Row doesn't fit  if nothing committed yet, force it (oversized row safety)
          if (segEnd < rowCursor) {
            tally += rowH;
            segEnd = r;
          }
 ...
 *        Removes tbody rows outside [startRow, endRow]
 *        Removes tfoot when !includesTfoot
...
const captureVirtualPage = async (
  ...
  const source = document.getElementById(containerId);
  if (!source) {
    console.warn(` captureVirtualPage: section "${containerId}" not found`);
    return null;
  }
  ...
  try {
    ...
    // Guard: if no [data-pdf-block] elements remain, the virtual page is empty  skip capture
    if (clone.querySelectorAll('[data-pdf-block]').length === 0) {
      console.warn(` Skipping empty virtual page (no visible blocks) for "${containerId}"`);
      return null;
    }
    ...
    const canvas = await html2canvas(clone, {
      ...
      onclone: (doc, clonedEl) => {
        ...
        logger.debug('[PDF] onclone (captureVirtualPage): ' + links.length + ' links');
      },
    });

    logger.debug(` Virtual page captured: ${canvas.width}${canvas.height}px`);
    const { canvas: composited, footerLinks } = await compositeWithPageFooter(canvas, source);
    return { canvas: composited, links: [...links, ...footerLinks] };
  } catch (error) {
    console.error(` captureVirtualPage failed for "${containerId}":`, error);
    throw error;
  } finally {
    document.body.removeChild(clone);
  }
...
const waitForPdfReady = async (sectionId: string): Promise<void> => {
  ...
  if (!readyEl) return; // no async content  nothing to wait for
  const MAX_WAIT = 15000;
  const POLL_INTERVAL = 150;
  const start = Date.now();
  logger.debug(` Waiting for ReferenceImages readiness: ${sectionId}...`);
  while (true) {
    if (readyEl.getAttribute('data-pdf-ready') === 'true') return;
    if (Date.now() - start >= MAX_WAIT) {
      console.warn(` waitForPdfReady: timeout reached for "${sectionId}"  continuing export`);
      return;
    }
...
const legacyFullCapture = async (
  ...
  logger.debug(' Using legacy full-element capture');
  ...
  while (remainingHeight > 0) {
    logger.debug(' pdf.addPage() called', {
      where: 'legacyFullCapture overflow',
      currentPageBefore: pdf.getCurrentPageInfo().pageNumber,
...
export const exportToPDF = async (
  ...
  try {
    console.group(' PDF EXPORT TRACE  ' + quoteNumber);
    logger.debug(' Starting PDF export process...');
    logger.debug('Element to capture:', element);
    ...
    logger.debug(' Device type:', isMobileDevice ? 'Mobile' : 'Desktop');
    ...
    const addLinkAnnotations = (
      ...
      links.forEach(({ url, x, y, w, h }) => {
        ...
        if (pdfW > 0 && pdfH > 0 && pdfY + pdfH > 0 && pdfY < pdfHeight) {
          ...
          logger.debug(
            `[PDF] link added page=${pageCount} url=${url} x=${pdfX.toFixed(2)} y=${finalY.toFixed(2)} w=${pdfW.toFixed(2)} h=${pdfH.toFixed(2)}`,
          );
        }
      });
    };
    ...
    // If content overflows A4 height it is scaled down proportionally to fit
    ...
    const addCanvasToPDF = (
      ...
      logger.debug(' addCanvasToPDF START', {
        label,
        isFirstPage,
        currentPdfPage: pdf.getCurrentPageInfo().pageNumber,
        canvasWidth: canvas.width,
        canvasHeight: canvas.height,
      });
      if (!isFirstPage) {
        logger.debug(' pdf.addPage() called', {
          where: 'addCanvasToPDF  not first page',
          label,
          currentPageBefore: pdf.getCurrentPageInfo().pageNumber,
          stack: new Error().stack,
        });
      ...
      if (imgHeight <= pdfHeight) {
        // Content fits within A4  render at natural size
        pdf.addImage(imgData, 'PNG', 0, 0, pdfWidth, imgHeight);
        addLinkAnnotations(links, 0, cssToPdf, 0);
        logger.debug(` Page ${pageCount} added (${label}): ${pdfWidth}${Math.round(imgHeight)}mm`);
      } else {
        ...
        while (remainingHeight > 0.5) {
          logger.debug(' pdf.addPage() called', {
            where: 'addCanvasToPDF overflow loop',
            label,
            currentPageBefore: pdf.getCurrentPageInfo().pageNumber,
            stack: new Error().stack,
          });
          ...
          remainingHeight -= pdfHeight;
          logger.debug(` Additional page ${pageCount} added for overflow`);
        }
      }
      logger.debug(' addCanvasToPDF END', {
        label,
        currentPdfPage: pdf.getCurrentPageInfo().pageNumber,
    ...
    if (isMultiService) {
      // --- MULTI-SERVICE QUOTE ---
      logger.debug(' Multi-service quote detected');

      //  Helper: smart-capture one section with overflow-aware block pagination
      // When scrollHeight > PDF_USABLE_HEIGHT_PX, measure blocks  virtual pages  capture each.
      // Otherwise falls back to captureSectionAtA4 (single capture, no splitting).
      const captureSmartSection = async (sectionId: string, label: string): Promise<boolean> => {
        const source = document.getElementById(sectionId);
        if (!source) return false;
        const isFirstOfDoc = pageCount === 0;
        logger.debug(` Smart pagination enabled: ${sectionId}`);
        if (source.scrollHeight > PDF_USABLE_HEIGHT_PX) {
          ...
          if (blocks.length > 0) {
            ...
            logger.debug(` Smart section START: ${sectionId}`, { virtualPages: vPages.length });
            logger.debug(` Virtual pages generated: ${vPages.length}`);
            for (let vp = 0; vp < vPages.length; vp++) {
              logger.debug(` Rendering virtual page ${vp + 1}/${vPages.length}`, {
                sectionId,
                currentPdfPage: pdf.getCurrentPageInfo().pageNumber,
              });
              logger.debug(` Capturing virtual page ${vp + 1}/${vPages.length}`);
              ...
              if (vpResult) {
                ...
                logger.debug(` Virtual page inserted`, {
                  sectionId,
                  vp,
                  insertedPdfPage: pdf.getCurrentPageInfo().pageNumber,
                });
              } else {
                logger.debug(` Virtual page ${vp + 1} returned null  skipped`, { sectionId });
              }
            }
            return true;
          }
      ...
      while (true) {
        ...
        if (refSource) {
          logger.debug(` Smart pagination enabled: pdf-service-ref-${serviceIndex}`);
          if (refSource.scrollHeight > PDF_USABLE_HEIGHT_PX) {
            ...
            if (refBlocks.length > 0) {
              ...
              logger.debug(` Virtual pages generated: ${refVPages.length}`);
              for (let vp = 0; vp < refVPages.length; vp++) {
                logger.debug(` Capturing virtual page ${vp + 1}/${refVPages.length}`);
                const vpResult = await captureVirtualPage(
                  `pdf-service-ref-${serviceIndex}`,
                  refVPages[vp],
                  refBlocks,
                );
                if (vpResult) {
                  addCanvasToPDF(
                    vpResult.canvas,
                    vpResult.links,
                    `service-ref-${serviceIndex}-vp${vp}`,
                    false,
                  );
                }
              }
      ...
      // Terms & Conditions (Last page - multi-service)  smart pagination
      await captureSmartSection('pdf-page-terms', 'terms');

      if (pageCount === 0) {
        console.warn(' No multi-service sections captured, using fallback');
        await legacyFullCapture(element, pdf, pdfWidth, pdfHeight);
        pageCount = 1;
      }
    } else {
      // --- SINGLE-SERVICE QUOTE ---
      logger.debug(' Capturing quotation content...');
      ...
      if (page1Source && page1Source.scrollHeight > PDF_USABLE_HEIGHT_PX) {
        // Overflow detected  smart block-row pagination
        logger.debug(
          ` pdf-page-1 overflows (${page1Source.scrollHeight}px > ${PDF_USABLE_HEIGHT_PX}px)  smart pagination`,
        );
        ...
        if (page1Blocks.length > 0) {
          ...
          logger.debug(
            ` [SmartPagination] pdf-page-1  ${page1VPages.length} virtual page(s):`,
            page1VPages,
          );
          ...
          // No blocks annotated  fall back to standard single capture
        ...
        // Fast path: section fits on one page  use existing capture unchanged
        const quotationResult = await captureSectionAtA4('pdf-page-1');
        if (quotationResult) {
          addCanvasToPDF(quotationResult.canvas, quotationResult.links, 'quotation', true);
        } else {
          console.warn(' Quotation section not found, using fallback full capture');
          await legacyFullCapture(element, pdf, pdfWidth, pdfHeight);
          pageCount = 1;
        }
      }
      ...
      logger.debug(' Checking for reference images...');
      ...
      if (page2Source) {
        if (page2Source.scrollHeight > PDF_USABLE_HEIGHT_PX) {
          ...
          if (page2Blocks.length > 0) {
            ...
            logger.debug(
              `[SmartPagination] pdf-page-2 => ${page2VPages.length} virtual page(s):`,
              page2VPages,
            );
          ...
          if (referenceResult && referenceResult.canvas.height > 10) {
            ...
            logger.debug(' No reference images section found or section is empty');
          }
        }
      }
      // General Terms & Conditions + Bank Details (final page)  smart pagination
      logger.debug(' Checking for general T&C + banking page...');
      const termsSource = document.getElementById('pdf-page-terms');
      if (termsSource) {
        logger.debug(` Smart pagination enabled: pdf-page-terms`);
        if (termsSource.scrollHeight > PDF_USABLE_HEIGHT_PX) {
          ...
          if (termsBlocks.length > 0) {
            ...
            logger.debug(` Virtual pages generated: ${termsVPages.length}`);
            for (let vp = 0; vp < termsVPages.length; vp++) {
              logger.debug(` Capturing virtual page ${vp + 1}/${termsVPages.length}`);
              const vpResult = await captureVirtualPage(
                'pdf-page-terms',
                termsVPages[vp],
                termsBlocks,
            ...
            if (generalTermsResult && generalTermsResult.canvas.height > 10) {
              addCanvasToPDF(
                generalTermsResult.canvas,
                generalTermsResult.links,
                'general-terms',
                false,
              );
            }
          ...
          if (generalTermsResult && generalTermsResult.canvas.height > 10) {
            ...
            logger.debug(' No general T&C/banking section found or section is empty');
          }
        }
      }
    }

    logger.debug(` PDF created with ${pageCount} pages`);
    ...
    logger.debug(' FINAL PDF PAGE COUNT', pdf.getNumberOfPages());
    logger.debug(' Saving PDF as:', filename);
    console.groupEnd(); // close  PDF EXPORT TRACE group

    // Handle mobile vs web differently
    if (isMobile()) {
      logger.debug(' Mobile detected - using Filesystem API');
      try {
        // Get PDF as base64
        ...
        logger.debug(' PDF saved to:', result.uri);
        ...
        try {
          ...
          logger.debug(' Download notification shown');
        } catch (notifError) {
          console.error(' Notification plugin not available:', notifError);
        ...
        try {
          ...
          logger.debug(' PDF auto-opened in default viewer');
        } catch (openError) {
          console.error(' Failed to auto-open PDF:', openError);
        ...
        logger.debug(' Falling back to browser download');
        pdf.save(filename);
      }
    } else {
      // Web browser - use normal download
      logger.debug(' Web detected - using browser download');
      pdf.save(filename);
    }
    ...
    console.groupCollapsed(' PDF EXPORT SUMMARY');
    console.table({
