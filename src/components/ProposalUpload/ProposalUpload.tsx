...
const ProposalUpload: React.FC = () => {
  ...
  const [duplicateInfo, setDuplicateInfo] = useState<{ fileName: string; uploadedAt: Date } | null>(
    null,
  );
  ...
  const headerBgOpen = useColorModeValue(
  ...
  const fileTypeBarGradient = useColorModeValue(
    'linear(to-b, brand.400, brand.600)',
    'linear(to-b, brand.300, brand.500)',
  );
  const fileIconBg = useColorModeValue(
    'linear(135deg, brand.50, brand.100)',
    'linear(135deg, brand.900, brand.800)',
  );
  ...
  const handleReplaceConfirm = async () => {
    ...
    try {
      ...
      if (cloudStorageEnabled) {
        try {
          const cloudDuplicate = await findCloudDuplicate(pendingFile.name, pendingFile.size);
          if (cloudDuplicate) {
            duplicate = cloudProposalToStored(cloudDuplicate);
          }
        } catch (error) {
  ...
  const actualProcessFile = async (file: File) => {
    ...
    if (fileType === 'pdf') {
      ...
      validation = {
        valid: false,
        error: 'Unsupported file type. Please upload PDF, JPEG, or Excel files only.',
      };
    }
  ...
  const processBatchFiles = async (files: File[]) => {
    ...
    logger.info(
      `📂 [BATCH] Starting upload of ${files.length} file(s):`,
      files.map((f) => f.name),
    );
    ...
    for (let i = 0; i < files.length; i++) {
      ...
      const file = files[i];
      logger.info(
        `📄 [BATCH ${i + 1}/${files.length}] Processing: ${file.name} (${(file.size / 1024 / 1024).toFixed(2)} MB)`,
      );
      ...
      try {
        if (cloudStorageEnabled) {
          ...
          const localDuplicate = await findDuplicateProposal(
            file.name,
            file.type || 'application/pdf',
            file.size,
          );
    ...
    logger.info(
      `📊 [BATCH] Done — ${processed} uploaded, ${skipped} skipped (duplicates), ${files.length - processed - skipped} failed`,
    );
    ...
    <>
              ...
              <VStack spacing={3}>
                ...
                <VStack spacing={1}>
                  ...
                  <Text fontSize="xs" color="gray.400">
                    PDF, JPEG, or Excel files (max 10MB each)
                  </Text>
                </VStack>
              </VStack>
            </Center>
          ) : (
            <Box p={4} borderWidth={1} borderColor={borderColor} borderRadius="lg" bg={bgColor}>
              <HStack spacing={3} justify="space-between" flexWrap="wrap">
                ...
                <Button size="sm" variant="outline" onClick={handleUploadClick} flexShrink={0}>
                  Change
                </Button>
              </HStack>
            </Box>
              ...
              >
                <HStack justify="space-between">
                  <HStack spacing={3}>
                    ...
                    <Center w={8} h={8} borderRadius="lg" bg={dragBg} color="brand.500">
                      ...
                      <HStack spacing={2}>
                        ...
                        {activeProposals.length > 0 && (
                          ...
                          >
                            ...
                            {recentProposals.filter((p) => p.fileType === 'application/pdf').length}{' '}
                            loaded
                          </Badge>
                        )}
                      </HStack>
                    ...
                    >
                      ...
                      <HStack spacing={4} align="start">
                        ...
                        <VStack align="start" spacing={1} flex={1} minW={0}>
                          ...
                          <HStack spacing={3} fontSize="xs" color={metaTextColor} flexWrap="wrap">
                            <HStack spacing={1}>
                        ...
                        <VStack spacing={2} flexShrink={0}>
                          {(() => {
                            const isLoaded = activeProposals.some(
                              (p) => p.id === storedProposal.id,
                            );
                            return (
                              <Button
                                size="sm"
                          ...
                          <IconButton
                            ...
                            colorScheme="red"
                            onClick={() =>
                              handleDeleteProposal(storedProposal.id, storedProposal.fileName)
                            }
                            isDisabled={loading}
                            borderRadius="lg"
                            _hover={{
      ...
      <Modal isOpen={isOpen} onClose={handleCancelUpload} isCentered>
        ...
        <ModalContent mx={4} borderRadius="xl">
          <ModalHeader pb={2}>
            <HStack spacing={3}>
              <Center w={10} h={10} borderRadius="lg" bg={modalIconBg}>
          ...
          <ModalBody py={4}>
            <VStack align="start" spacing={3}>
              <Box p={3} bg={modalBoxBg} borderRadius="lg" w="full">
                <VStack align="start" spacing={1}>
                  ...
                  <Text fontSize="xs" color="gray.500" pl={6}>
                    Uploaded{' '}
                    {duplicateInfo?.uploadedAt ? formatRelativeTime(duplicateInfo.uploadedAt) : ''}
                  </Text>
                </VStack>
              </Box>
              ...
              >
                <Text fontSize="xs" color={modalNoteColor}>
                  <strong>Note:</strong> Replacing will delete the old file and upload the new
                  version.
                </Text>
              </Box>
            </VStack>
          ...
          <ModalFooter pt={2}>
            <HStack spacing={3} w="full">
              <Button flex={1} variant="outline" onClick={handleCancelUpload} borderRadius="lg">
                Cancel
              </Button>
              <Button
                flex={1}
