...
export const PremiumAgency: React.FC<TemplateProps> = ({
  data,
  ...
  const renderTermWithLinks = (term: string): React.ReactNode => {
    ...
    while ((match = urlPattern.exec(term)) !== null) {
      ...
      parts.push(
        <a key={match.index} href={match[0]} style={{ color: 'inherit', textDecoration: 'none' }}>
          {match[0]}
        </a>,
      );
  ...
  return (
    <>
      <div
        id={isMultiService ? 'pdf-page-summary' : 'pdf-page-1'}
        className="template-premium-agency"
      >
        {/* Header */}
        <div className="pa-header">
          <div className="pa-header-left">
            {company.logo && <img src={company.logo} alt={company.name} className="pa-logo" />}
              ...
              <table className="pa-table">
                  ...
                  {quote.items.map((item, index) => {
                    ...
                    return (
                      <tr key={index}>
                        ...
                        {quote.gstEnabled && (
                          <td className="pa-cell-gst-pct">{quote.gstPercentage}%</td>
                        )}
                        {quote.gstEnabled && (
                          <td className="pa-cell-incl">{formatCurrency(itemFinal)}</td>
                        )}
                ...
                <tfoot>
                  <tr className="tfoot-totals">
                    <td
                      className="tfoot-label"
                      colSpan={quote.items.some((i) => i.duration && i.duration > 1) ? 5 : 4}
                    >
                      Total
                    </td>
                    ...
                    {quote.gstEnabled && (
                      <td className="tfoot-incl">{formatCurrency(calculateTotal())}</td>
                    )}
                    {hasRemark && <td></td>}
                  </tr>
                </tfoot>
          ...
          {quote.items.map((item) =>
            item.termsAndConditions ? (
              <div key={item.id} className="pa-terms">
                <h3 className="pa-section-title">
                  {item.description.split(' - ')[0]} — Terms & Conditions
                </h3>
                <div className="pa-terms-grid">
                  {item.termsAndConditions
                    .split(/\n|•|\d+\.\s/)
          ...
          return (
            <React.Fragment key={groupIndex}>
                      ...
                      <table className="pa-table">
                        ...
                        <tbody>
                          {group.items.map((item, index) => {
                            const itemGST = quote.gstEnabled
                              ? item.total * (quote.gstPercentage / 100)
                              : 0;
                            const itemFinal = item.total + itemGST;
                            return (
                              <tr key={index}>
                                <td className="pa-cell-num">
                                  {String(index + 1).padStart(2, '0')}
                                </td>
                                <td className="pa-cell-desc">
                                  <strong>{item.description}</strong>
                                  {item.details && (
                                    <div className="pa-item-details">{item.details}</div>
                                  )}
                                </td>
                                <td className="pa-cell-qty">{item.quantity}</td>
                                <td className="pa-cell-rate">{formatCurrency(item.rate)}</td>
                                ...
                                {hasRemarkGroup && (
                                  <td className="pa-cell-remark">{item.remark || ''}</td>
                                )}
                              </tr>
                            );
                          })}
                        ...
                        <tfoot>
                          <tr className="tfoot-totals">
                            <td className="tfoot-label" colSpan={4}>
                              Total
                            </td>
                            <td className="tfoot-excl">{formatCurrency(groupSubtotal)}</td>
                            {quote.gstEnabled && <td className="tfoot-gst-pct"></td>}
                            {quote.gstEnabled && (
                              <td className="tfoot-incl">{formatCurrency(groupTotal)}</td>
                            )}
              ...
              {(group.termsAndConditions || quote.termsAndConditions) && (
                <div
                  id={`pdf-service-specific-terms-${groupIndex}`}
                  className="template-premium-agency"
                >
                  <div className="pa-terms">
                    <h3
