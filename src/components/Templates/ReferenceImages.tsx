...
import { extractReviewViaGemini, cropReferencePageImage, cropPageStrippingHeaderFooter, cropSpecAboveReference, cropSpecDiagram } from '../../utils/pdfUtils';
...
function filterPagesByCategory(pages: ExtractedPage[], category: string): ExtractedPage[] {
  ...
  // Extract meaningful keywords, excluding pricing/quantity terms and common noise words
  const words = serviceTypeOnly
    ...
    .filter((w) => 
      w.length >= 3 && 
      !['rental', 'price', 'per', 'month', 'and', 'the', 'for', 'from', 'display', 'printing', 'fixing',
        'side', 'unit'].includes(w)
    );
  ...
  const matchedPages = pages.filter((page) => {
    ...
    if (isReferenceImagePage) {
      console.log(`✅ Page ${page.pageNumber} - REFERENCE IMAGE page detected (heading matched)!`);
      return true;
    }
...
function extractAllServiceTypes(items: QuoteItem[]): string[] {
  ...
  for (const item of items) {
    ...
    if (!extracted) {
      const vehicleMatch = desc.match(/(bus|auto|tempo|cab|apartment|building|lift|elevator|residential|commercial)\s+(full|semi|back|panel|shelter|rickshaw|branding|sticker|stickers|poster|posters|non|led)(\s+\w+)*/i);
      if (vehicleMatch) extracted = vehicleMatch[0].replace(/\s*[-–—].*$/, '').trim();
    }
...
export const ReferenceImages: React.FC<ReferenceImagesProps> = ({ proposalPages, proposalPageMap, items }) => {
  ...
  useEffect(() => {
    ...
    let cancelled = false;
    console.log('🔍 Triggering content-aware spec crop for shared spec/ref page', specPage.pageNumber);
    cropSpecAboveReference(specPage.imageDataUrl).then(cropped => {
      if (!cancelled) {
        console.log('✅ Lazy spec crop done for page', specPage.pageNumber);
